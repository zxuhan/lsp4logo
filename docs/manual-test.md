...
### 4.3 Completion (Phase 8)

- Open an empty line inside a `TO` body. Type **Ctrl-Space**. You should see:
  keywords (TO, END, REPEAT, …), built-ins (FORWARD, FD, BACK, BK, …), and any
  user-defined procedures in the file.
...
Open a new file and type `HAMBURGER 1` → red squiggle "unknown procedure: HAMBURGER".

Edit `TO foo :x\n  FD 100\nEND` → yellow squiggle on `:x`: "unused parameter: x".

### 4.6 Outline view (Phase 10)
